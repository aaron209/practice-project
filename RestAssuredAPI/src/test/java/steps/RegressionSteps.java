...
public class RegressionSteps extends BaseTest {
    ...
    public void the_client_sends_request_with_body(String applicationName, String apiPath) {
        ...
        try {
            ...
            response = given().spec(requestSpecification).body(body).when().post(PropertiesReader.readConfigFile("Resource")).then().spec(responseSpecification).extract().response();
